...
除手改 `.env` 外，管理员可在**管理后台 → 设置**页签（`/api/v1/admin/dashboard`）在线修改大部分配置项，无需手动编辑文件：

- 覆盖范围：基础（部署模式/CORS/关机宽限）、LLM、ASR、TTS、唤醒、音乐、OTA、表情、性能限流等约 50 项
- **即时生效**：唤醒语、音乐、OTA、表情、成长任务冷却等运行时动态读取的配置，保存后立即热应用
- **需重启**：LLM/ASR/TTS 服务商密钥与模型、性能参数等启动时快照的配置，保存后提示需重启完全生效
- **密钥安全**：API Key 类字段只显示「已设置/未设置」，不回显明文；编辑时留空表示保持现有值

对应接口（均需管理员权限）：

| 接口 | 说明 |
|------|------|
| `GET /api/v1/admin/settings` | 按分组返回全部可编辑设置项及当前值（密钥掩码） |
| `PUT /api/v1/admin/settings` | 白名单校验后写入 `.env` 并热应用内存配置，返回需重启项列表 |

写 `.env` 时值会自动做引号转义（多行系统提示词安全支持），并记录到管理员操作日志。
